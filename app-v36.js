...
const FREE_DEMO_BARS = Infinity; // Versión completa: sin límite de compases.
const MIN_BPM = 40;
const MAX_BPM = 240;
...
const VISUAL_LATENCY_FALLBACK_SECONDS = 0.035;
...
let percussionSampleBuffers = { madera: [], cajon: [] };
let percussionSampleIndex = { madera: 0, cajon: 0 };
...
async function ensureAudioReady() {
  ...
  await Promise.all([loadPalmasSamples(), loadPercussionSamples()]);
...
async function loadPercussionSamples() {
  await Promise.all(
    Object.entries(PERCUSSION_SAMPLE_FILES).map(async ([name, urls]) => {
      if (percussionSampleBuffers[name]?.length) return;
      ...
      const loadedBuffers = await Promise.all(
        urls.map(async (url) => {
          try {
            ...
            console.warn(`${url} no está disponible. Se omitirá esa muestra.`, error);
            return null;
          }
        })
      );

      percussionSampleBuffers[name] = loadedBuffers.filter(Boolean);
    })
  );
}
...
function scheduleBeat(beatIndex, time) {
  ...
  const beatVolume = palo.beatVolumes?.[beatLabel] ??
    (isAccent
      ? palo.accentVolume ?? 1
      : isWeak
        ? palo.weakBeatVolume ?? WEAK_BEAT_VOLUME
        : palo.pulseVolume ?? 1);

  if (!isRest) playSound(isAccent ? "accent" : "pulse", time, isAccent, beatVolume);
  ...
  queueVisualBeat(beatIndex, time, playSessionId);
}

function queueVisualBeat(beatIndex, time, sessionId) {
  visualBeatQueue.push({ beatIndex, time: time + getVisualLatencyCompensation(), sessionId });
}

function getVisualLatencyCompensation() {
  if (!audioContext) return VISUAL_LATENCY_FALLBACK_SECONDS;
  return audioContext.outputLatency || audioContext.baseLatency || VISUAL_LATENCY_FALLBACK_SECONDS;
}

function startVisualClock(sessionId) {
  stopVisualClock();
  visualBeatQueue = [];

  const tick = () => {
    if (!isPlaying || sessionId !== playSessionId || !audioContext) return;

    const audioNow = audioContext.currentTime;
    let activeBeat = null;

    while (visualBeatQueue.length && visualBeatQueue[0].time <= audioNow + 0.004) {
      const event = visualBeatQueue.shift();
      if (event.sessionId === sessionId) activeBeat = event.beatIndex;
    }

    if (activeBeat !== null) updateVisualBeat(activeBeat);
    visualFrameId = window.requestAnimationFrame(tick);
  };
...
function stopVisualClock() {
  if (visualFrameId) window.cancelAnimationFrame(visualFrameId);
  visualFrameId = null;
  visualBeatQueue = [];
}
...
function playSampledPercussion(kind, time, isAccent, volume = 1) {
  const buffers = percussionSampleBuffers[kind] || [];
  const buffer = getNextPercussionBuffer(kind, buffers);
  ...
  gain.gain.setValueAtTime((isAccent ? 0.98 : 0.6) * volume * getHumanGain(), time);
  source.connect(filter).connect(gain).connect(audioContext.destination);
  source.start(time);
}

function getNextPercussionBuffer(kind, buffers) {
  if (!buffers.length) return null;
  const index = percussionSampleIndex[kind] % buffers.length;
  percussionSampleIndex[kind] = index + 1;
...
function playSampledPalmas(time, isAccent, volume = 1) {
  ...
  source.buffer = buffer;
  source.playbackRate.setValueAtTime(1, time);
  gain.gain.setValueAtTime((isAccent ? 1 : 0.72) * volume * getHumanGain(), time);
  source.connect(gain).connect(audioContext.destination);
  source.start(time);
}

function getHumanGain() {
  return 0.94 + Math.random() * 0.09;
}
...
function updateVisualBeat(activeIndex) {
  elements.beatGrid.querySelectorAll(".beat").forEach((beat) => {
    const isActive = Number(beat.dataset.index) === activeIndex;
    beat.classList.toggle("active", isActive);
    beat.classList.toggle("active-accent", isActive && beat.classList.contains("accent"));
    beat.classList.toggle("active-weak", isActive && beat.classList.contains("weak"));
  });
}
